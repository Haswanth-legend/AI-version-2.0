...
<head>
  ...
  <title>AI ASSISTANT — All in one hand</title>
  ...
  <aside>
    <div class="logo">
      ...
      <div>
        ...
        <div class="small">All in one hand — Professional</div>
      </div>
    </div>
    <nav id="sidebarNav">
      <a data-target="dashboard" class="active">🏠 Dashboard</a>
      <a data-target="tasks">⏰ Tasks & Reminders</a>
      <a data-target="water">💧 Water Tracker</a>
      <a data-target="meds">💊 Medication Reminder</a>
      <a data-target="calc">🧮 Calculator</a>
      <a data-target="tutorials">📚 Tutorials</a>
      <a data-target="creators">🎨 Creators</a>
      <a data-target="profile">👤 Profile</a>
      <a data-target="settings">⚙️ Settings</a>

      <!-- NEW sections (kept styling consistent) -->
      <a data-target="calendar">📅 Calendar & Events</a>
      <a data-target="fitness">💪 Fitness Tracker</a>
      <a data-target="mood">🧘 Mood Journal</a>
      <a data-target="notes">📓 Notes</a>
      <a data-target="planner">💼 Daily Planner</a>
    </nav>
    <footer>
    ...
    <section class="grid">
      <div id="mainColumn">
        ...
        <div class="card" id="dashboardSection">
          <div style="display:flex;gap:12px;align-items:flex-start">
            <div style="flex:1">
              ...
              <div class="muted" id="overviewInfo">No reminders for now — create tasks, water goals, or meds and we'll remind you loudly when the time comes.</div>
              <div style="display:flex;gap:10px;margin-top:12px">
                <div style="flex:1" class="card">
                  <h3 style="margin:0">Next Reminder</h3>
                  <div id="nextReminder" class="muted">—</div>
                </div>
                <div style="width:170px" class="card">
                  <h3 style="margin:0">Water today</h3>
                  <div id="waterSummary" class="muted">—</div>
                </div>
              </div>
            ...
            <div style="width:320px">
              <div class="motivation card">
                ...
                <div id="motivationText" style="font-weight:700;font-size:15px">Stay focused — your assistant is ready.</div>
                <div class="muted" style="margin-top:8px" id="motivationAuthor">—</div>
        ...
        <div class="card" id="medsSection" style="margin-top:14px">
          ...
          <div class="muted">Add medication entries — they show in a neat table and will remind you.</div>
          ...
          <div style="display:flex;gap:12px;margin-top:12px">
            <div class="creator-card">
              ...
              <div>
                ...
                <div class="muted">Grade 9 — Lead Developer & Designer</div>
            ...
            <div class="creator-card">
              ...
              <div>
                ...
                <div class="muted">Grade 9 — Features & QA</div>
        ...
        <div class="card" id="moodSection" style="margin-top:14px;display:none">
          <h2>Mood Journal</h2>
          <div class="muted">Quick daily mood logs — emoji + short note.</div>
          <div style="margin-top:12px;display:flex;gap:12px;align-items:center">
            <div class="mood-grid" id="moodBtns">
              <div class="mood-btn" data-emoji="😄">😄 Happy</div>
              <div class="mood-btn" data-emoji="🙂">🙂 Fine</div>
              <div class="mood-btn" data-emoji="😐">😐 Neutral</div>
              <div class="mood-btn" data-emoji="😔">😔 Sad</div>
              <div class="mood-btn" data-emoji="😡">😡 Angry</div>
            </div>
            <div style="flex:1">
        ...
        <div class="card" id="notesSection" style="margin-top:14px;display:none">
          <h2>Notes</h2>
          <div class="muted">Quick notes — autosaved locally.</div>
          <div style="display:flex;gap:12px;margin-top:12px">
            <div style="flex:1">
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  /* -------------- Motivation panel -------------- */
  const quotes = [
    {q:"Small progress is still progress. Keep going.", a:"— Anonymous"},
    {q:"Learn something new today; your future self will thank you.", a:"— You"},
    {q:"Focus on what you can control. Let the rest go.", a:"— Stoic Tip"},
    {q:"Consistency compounds. A little each day adds up.", a:"— Habit Coach"},
    {q:"You are closer than you think — take the next step.", a:"— Motivator"}
  ];
  ...
  function openTutorialsPage(){
    const content = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Tutorials — AI Assistant</title><style>body{font-family:Inter,Arial;background:linear-gradient(180deg,#071026 0%,#07162a 60%);color:#e6eef8;padding:18px}h1{margin-bottom:6px}section{background:linear-gradient(180deg,rgba(255,255,255,0.02),transparent);padding:18px;border-radius:12px;margin-bottom:12px}ul{color:#cbd5e1}</style></head><body><h1>Tutorials — Step by step</h1><p style="color:#aab8d6">Practical guides and examples to use your assistant.</p>` +
    '<section><h2>Reminders</h2><p>Open Tasks → enter Title & Date/Time → Add. Allow notifications when asked. When reminder fires, a modal appears with Dismiss or Snooze (10 minutes).</p></section>' +
  ...
  function renderWater(){
    ...
    if(!state.water || !state.water.dailyLitres){ el.innerHTML = '<div class="muted">No goal set</div>'; $('waterSummary').innerText='—'; return; }
    let html = `<div class="small">Goal: ${state.water.dailyLitres} L — plan:</div><table><thead><tr><th>When</th><th>Amount</th></tr></thead><tbody>`;
  ...
  /* -------------- Mood -------------- */
  function renderMoodUI(){ const todayISO = new Date().toISOString().slice(0,10); const t = (state.moods||[]).find(m=>m.date===todayISO); $('todayMood').innerText = t ? `${t.emoji} ${t.note? '— ' + t.note : ''} · ${new Date(t.when).toLocaleTimeString()}` : 'No mood logged today'; }
  ...
  $('viewMoodHistory').addEventListener('click', ()=>{ const h = $('moodHistory'); if(h.style.display==='block'){ h.style.display='none'; return; } const items = (state.moods||[]).slice().reverse().map(m=>`<div style="padding:8px;border-bottom:1px dashed rgba(255,255,255,0.03)">${m.emoji} <strong>${m.date}</strong> — ${m.note||''}</div>`).join(''); h.innerHTML = items || '<div class="muted">No moods yet</div>'; h.style.display='block'; });
  ...
  function renderAll(){
    ...
    $('nextReminder').innerText = pending.length ? (pending[0].title + ' — ' + (isNaN(new Date(pending[0].when))?'-':new Date(pending[0].when).toLocaleString())) : '—';
    $('overviewInfo').innerText = pending.length ? `You have ${pending.length} upcoming reminders` : 'No reminders for now — create tasks, water goals, or meds and we\'ll remind you loudly when the time comes.';
  }
